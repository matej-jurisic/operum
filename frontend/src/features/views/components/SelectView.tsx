...
export default function SelectViewMenu() {
    ...
    const sortConflicts = useMemo(() => {
        ...
        const conflicts: { fieldName: string; skippedView: string; keptView: string }[] = [];

        for (const view of activeViews) {
            for (const sort of [...view.sorts].sort((a, b) => a.order - b.order)) {
                ...
                {sortConflicts.length > 0 && (
                    <>
                        ...
                        {sortConflicts.map((c, i) => (
                            <Menu.Item key={i} disabled style={{ cursor: "default" }}>
